...
class DCE(nn.Module):
    ...
    def __init__(self,  num_class=7,epsilon=0.35,device=None, reduction="mean"):
    ...
    def forward(self, prediction, target_label,clean, one_hot=True):
        ...
        N,C = y_pred.shape
        smooth_labels = torch.full(size=(N, C), fill_value=self.epsilon/(C - 1)).to(self.device)
...
def cross_entropy(logits, labels, reduction='mean'):
    ...
    #print('losses :',losses)
    #print('torch.sum(losses)  :',torch.sum(losses))
